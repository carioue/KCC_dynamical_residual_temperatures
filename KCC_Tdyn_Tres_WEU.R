...
x_fit_be_nonat = function(Xd,x_df,ant=T) {
	...
	for (mod in Models){
		message(c("			",mod))
	   x = Xd[,mod]
	   x_ant_uncentered_be = HatM %*% x								# mu + f(t)
	   x_ant_be = x_ant_uncentered_be - x_ant_uncentered_be[1]	# f(t)
	   X[,"ant",mod] = x_ant_be
		X[,"all",mod] = mean(x) + x_ant_be - mean(x_ant_be)	# = 1 + Enat + f(t)
...
year_l = 1900:2100
ny_l = length(year_l)
era5_dyn = nc_open('Tdyn_era5_day_1970_2024_yearmean_fldmean_WEU_land.nc')
era5_res = nc_open('Tres_era5_day_1970_2024_yearmean_fldmean_WEU_land.nc')
...
if (do_antnat_dec_loc == T) {
	...
	df = 6                            
	message("Decomposition for each model (take some time!)...")
	X_fit_loc_dyn[,c("all","nat"),] = x_fit_be_nonat(X_loc_scen_dyn[,,"value"],df, ant=F)
	X_fit_loc_res[,c("all","nat"),] = x_fit(X_loc_scen_res[,,"value"], e_loc, df, ant=F)
	save("X_fit_loc_dyn",file=("X_fit_loc_dyn_noNAT.Rdata"))
	save("X_fit_loc_res",file=("X_fit_loc_res.Rdata"))
	
} else {

	load(paste0("X_fit_loc_dyn_noNAT.Rdata"))
...
if (do_fit_MAR_obs_loc == T) {
	...
	save(res_loc, theta_obs_loc, Sigma2_obs_iv_loc_dyn, file=paste0("MAR_obs_tas_loc_dyn_Xo19702024.Rdata"))
	...
	save(res_loc, theta_obs_loc, Sigma2_obs_iv_loc_res, file=paste0("MAR_obs_tas_loc_res_Xo19702024.Rdata"))
	
} else {

	load(paste0("MAR_obs_tas_loc_dyn_Xo19702024.Rdata"))
	load(paste0("MAR_obs_tas_loc_res_Xo19702024.Rdata"))
...
X_krig_loc_glo_list = prior2posterior(X_fit, Xo, Sigma2_obs_loc_glo, Nres, centering_CX=T, ref_CX=c(paste0(1970:2024,"locdyn"),paste0(1970:2024,"locres"),paste0(1961:1990,"glo"))) 
...
save(X_krig_tdyn, file=paste0("KCC_Tdyn_WEU_noNAT.Rdata"))
